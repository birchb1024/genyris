// Copyright 2008 Peter William Birch <birchb@genyis.org>
//
// This software may be used and distributed according to the terms
// of the Genyris License, in the file "LICENSE", incorporated herein by reference.
//
package org.genyris.interp.builtin

import org.genyris.core.Exp
import org.genyris.core.Pair
import org.genyris.core.StrinG
import org.genyris.core.Symbol
import org.genyris.exception.GenyrisException
import org.genyris.interp.ApplicableFunction
import org.genyris.interp.Closure
import org.genyris.interp.Environment
import org.genyris.interp.Interpreter

class CatchFunction(interp: Interpreter) : ApplicableFunction(interp, "catch", false) {
    @kotlin.Throws(GenyrisException::class)
    override fun bindAndExecute(proc: Closure?, arguments: Array<Exp?>, env: Environment): Exp? {
        var retval: Exp? = NIL
        this.checkMinArguments(arguments, 2)
        if (arguments[0] is Symbol) {
            val errorVar = arguments[0] as Symbol
            env.defineVariable(errorVar, NIL)
            val body = arrayToList(arguments).cdr()
            try {
                retval = body.evalSequence(env)
            } catch (e: StackOverflowError) {
                _interp.resetDebugBackTrace()
                env.setVariableValue(errorVar, StrinG("Stack overflow."))
            } catch (e: GenyrisException) {
                _interp.resetDebugBackTrace()
                env.setVariableValue(errorVar, e.getData())
            }
            return retval
        } else if (arguments[0] is Pair) {
            if (arguments[0]!!.car() !is Symbol) {
                throw GenyrisException(
                    (this.getName()
                            + " was expecting a list containing two symbols, but got "
                            + arguments[0]!!.car().toString())
                )
            }
            val errorVar = arguments[0]!!.car() as Symbol?
            if (arguments[0]!!.cdr() !is Pair) {
                throw GenyrisException(
                    (this.getName()
                            + " was expecting a list containing two symbols, but got "
                            + arguments[0]!!.cdr().toString())
                )
            }
            if (arguments[0]!!.cdr().car() !is Symbol) {
                throw GenyrisException(
                    (this.getName()
                            + " was expecting a list containing two symbols, but got "
                            + arguments[0]!!.cdr().car().toString())
                )
            }
            val backTraceVar = arguments[0]!!.cdr().car() as Symbol?
            env.defineVariable(errorVar, NIL)
            env.defineVariable(backTraceVar, NIL)
            val body = arrayToList(arguments).cdr()
            try {
                retval = body.evalSequence(env)
            } catch (e: StackOverflowError) {
                env.setVariableValue(errorVar, StrinG("Stack overflow."))
                env.setVariableValue(backTraceVar, _interp.getDebugBackTraceAsList())
                _interp.resetDebugBackTrace()
            } catch (e: GenyrisException) {
                env.setVariableValue(errorVar, e.getData())
                env.setVariableValue(backTraceVar, _interp.getDebugBackTraceAsList())
                _interp.resetDebugBackTrace()
            }
            return retval
        } else {
            throw GenyrisException(
                (this.getName()
                        + " was expecting a symbol, or a pair of symbols but got "
                        + arguments[0].toString())
            )
        }
    }
}
