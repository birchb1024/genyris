...
class DefFunction(interp: Interpreter) : ApplicableFunction(interp, "def", false) {
    ...
    override fun bindAndExecute(
        ...
        val lambdaExpression: Exp = Pair(_lambda, arrayToList(arguments).cdr())
        val fn = EagerProcedure(
            envForBindOperations, lambdaExpression,
            ClassicFunction(arguments[0].toString(), _interp)
        )
