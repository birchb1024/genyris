...
class MatchMethod(interp: Interpreter?) : AbstractStringMethod(
    interp,
    staticName
) {
    ...
    override fun bindAndExecute(proc: Closure?, arguments: Array<Exp?>, env: Environment?): Exp? {
        if (arguments.size > 0) {
            if (arguments[0] !is StrinG) {
                throw GenyrisException("Non string passed to " + Constants.MATCH)
            }
            val regex = arguments[0] as StrinG
            val theString = getSelfString(env)
            return theString.match(NIL, TRUE, regex)
        } else {
            throw GenyrisException("Missing argument to " + Constants.MATCH)
        }
    }
