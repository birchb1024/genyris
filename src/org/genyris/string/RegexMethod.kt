...
class RegexMethod(interp: Interpreter?) : AbstractStringMethod(
    interp,
    staticName
) {
    ...
    override fun bindAndExecute(proc: Closure?, arguments: Array<Exp?>, env: Environment?): Exp? {
        if (arguments.size > 0) {
            if (arguments[0] !is StrinG) {
                throw GenyrisException("Non string passed to " + Constants.REGEX)
            }
            val regex = arguments[0] as StrinG
            val theString = getSelfString(env)
            return theString.regex(NIL, TRUE, regex)
        } else {
            throw GenyrisException("Missing argument to " + Constants.REGEX)
        }
    }
