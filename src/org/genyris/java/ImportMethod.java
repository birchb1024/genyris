...
public class ImportMethod extends AbstractJavaMethod {
	...
	public Exp bindAndExecute(Closure proc, Exp[] arguments,
		...
		StandardClass glass = StandardClass.makeClass(_interp.getGlobalEnv(),
				_interp.intern(new EscapedSymbol(klassName)), new Pair(_interp
						.intern(Constants.JAVA), NIL));
		//
		...
		for (int c = 0; c < ctors.length; c++) {
			...
			JavaCtor gctor = new JavaCtor(_interp, glass, name, ctors[c],
					params);
			glass.defineDynamicVariable(
					new DynamicSymbol(_interp.intern(name)),
		...
		for (int i = 0; i < methods.length; i++) {
			...
			if (Modifier.isStatic(methods[i].getModifiers())) {
				gmethod = new JavaStaticMethod(_interp, name, methods[i], params);
			} else {
				gmethod = new JavaMethod(_interp, name, methods[i], params);
			}
			glass.defineDynamicVariable(
					new DynamicSymbol(_interp.intern(name)),
					new EagerProcedure(envForBindOperations, NIL, gmethod));
