...
class ReaderStreamTest : TestCase() {
    @kotlin.Throws(Exception::class)
    fun testReadFileDigest() {
        val rs = ReaderStream(
            InputStreamReader(
                FileInputStream("test/fixtures/secret.txt")
            ), "fixtures/secret.txt"
        )
        Assert.assertEquals("5ebe2294ecd0e0f08eab7690d2a6ee69", rs.digest("MD5"))
    }

    @kotlin.Throws(Exception::class)
    fun testCopy() {
        val input = "1234567"
        val rs = ReaderStream(input)
        val out: Writer = StringWriter()
        rs.copy(out)
        System.out.println(out)
        Assert.assertEquals(input, out.toString())
    }

    @kotlin.Throws(GenyrisException::class)
    private fun testDigest(type: String?, `in`: String?, expected: String?) {
        val rs = ReaderStream(`in`)
        Assert.assertEquals(expected, rs.digest(type))
    }

    @kotlin.Throws(Exception::class)
    fun testStringDigests() {
