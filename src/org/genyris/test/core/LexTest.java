...
public class LexTest extends TestCase {
	...
	private void excerciseNextTokenInt(Exp expected, String toparse)
	...
	private void excerciseNextTokenBignum(Exp expected, String toparse)
			throws GenyrisException {
		_table.init(null);
		Lex lexer = new Lex(
				new UngettableInStream(new StringInStream(toparse)), _table);
	...
	private void excerciseNextTokenExp2Times(Exp expected, String toparse)
			throws GenyrisException {
		_table.init(null);
		Lex lexer = new Lex(
				new UngettableInStream(new StringInStream(toparse)), _table);
	...
	private void excerciseNextTokenExp(Exp expected, String toparse)
			throws GenyrisException {
		_table.init(null);
		Lex lexer = new Lex(
				new UngettableInStream(new StringInStream(toparse)), _table);
	...
	public void testLexIdentEscaped() throws Exception {
		...
		excerciseNextTokenExp2Times(new SimpleSymbol("q|"), "|werw |q|");
		excerciseNextTokenExp(new EscapedSymbol("with a space in it"),
				"|with a space in it|");
		excerciseNextTokenExp(new EscapedSymbol("http://foo/bar space/#123"),
				"|http://foo/bar space/#123|");
		excerciseNextTokenExp(new URISymbol("http://foo/bar%20space/#123"),
		...
		excerciseNextTokenExp(new EscapedSymbol("123"), "  \t|123|");
		excerciseNextTokenExp(new URISymbol("http://foo/bar#123"),
				"|http://foo/bar#123|");
		excerciseNextTokenExp(new URISymbol("http://foo/b:ar#123"),
				"|http://foo/b\\:ar#123|");
	...
	public void testCombination1() throws Exception {
		Lex lexer = new Lex(new UngettableInStream(new StringInStream(
				"int 12 double\n 12.34\r\n -12.34e5 \"string\" ")), _table);
		assertEquals(new SimpleSymbol("int").toString(), lexer.nextToken()
				.toString());
		assertEquals(new Bignum("12"), lexer.nextToken());
		assertEquals(new SimpleSymbol("double").toString(), lexer.nextToken()
				.toString());
	...
	private void excerciseListParsingLisp(String toParse, String expected)
			throws Exception {
		SimpleSymbol NIL = new NilSymbol();
		SymbolTable table = new SymbolTable();
		table.init(NIL);
