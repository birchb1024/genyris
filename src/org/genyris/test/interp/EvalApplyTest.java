...
public class EvalApplyTest extends TestCase {
	...
	void excerciseEval(String exp, String expected, String exceptionExpected)
			throws Exception {
		Interpreter interp = new Interpreter();
		Environment env = interp.getGlobalEnv();
	...
	public void testLambdaq1() throws Exception {
		...
		excerciseEval("((lambdaq (s) (cons 1 s)) (cons foo bar))",
				"(1 cons foo bar)");
	}

	public void testSequence() throws Exception {
		excerciseEval("((lambda (x) (cons x x) (cons 3 4)) 23)", "(3 = 4)");
		excerciseEval("((lambda (x y) (cons x x) (cons y x)) 23 45)",
				"(45 = 23)");
	}
	...
	public void testSet() throws Exception {
		...
		excerciseEval("((lambda (x) (set (quote x) 99) (cons x x)) 23)",
				"(99 = 99)");
	}
	...
	public void testCatch() throws Exception {
		...
		excerciseEval("(or (catch x qweqweqwe) x)",
	...
	public void testRaise() throws Exception {
		...
		excerciseEval("(or (catch x (raise (dict (.a =3)))) x)",
				"(dict (.a = 3))");
		excerciseEval("(or (catch x (raise 1)) x)", "1");
		excerciseEval("(or (catch x (raise ^w)) x)", "w");
		excerciseEval("(or (catch x (raise ^(1))) x)", "(1)");
