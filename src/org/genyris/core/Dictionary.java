...
public class Dictionary extends Atom implements Environment {
    ...
    public Exp dir(Internable table) {
        ...
        Exp result = Pair.cons3(new DynamicSymbol(table.SELF()),
                new DynamicSymbol(table.VARS()),
                new DynamicSymbol(table.CLASSES()), table.NIL());
        while (iter.hasNext()) {
            Exp key = new DynamicSymbol((SimpleSymbol) iter.next());
    ...
    public Exp lookupDynamicVariableValue(DynamicSymbol dsymbol) throws UnboundException {
        Symbol symbol = dsymbol.getRealSymbol();
    ...
    public Dictionary addProperty(Environment env, String name, Exp value) throws GenyrisException {
        defineDynamicVariable(new DynamicSymbol(env.internString(name)),
                value);
        return this;
    }

    public void defineInt(String name, int i) throws GenyrisException {
        defineDynamicVariable(new DynamicSymbol(internString(name)), new Bignum(i));
    }

    public void defineString(String name, String string2) throws GenyrisException {
        defineDynamicVariable(new DynamicSymbol(internString(name)), new StrinG(string2));
    }

    public void defineSymbol(String name, String string2) throws GenyrisException {
        defineDynamicVariable(new DynamicSymbol(internString(name)), internString(string2));
    }
